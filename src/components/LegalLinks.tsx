...
const termsSections = (): Section[] => [
  ...
  },
  {
    ...
    body: [
      ...
      {
        en: `No purchase is needed to take part: flights are free (Stardust is earned by playing). Limits: ${DROP_RULES.dropsPerWalletPerWeek} coin per wallet per week and only the first ${DROP_RULES.eligibleFlightsPerDay} flights of the day take part. The coin must be caught during the flight and the flight completed with at least ${Math.round(DROP_RULES.minQuality * 100)}% of the route score; the server checks the finished flight before recording the prize.`,
        pt: `Não é preciso comprar nada para participar: os voos são grátis (Stardust se ganha jogando). Limites: ${DROP_RULES.dropsPerWalletPerWeek} moeda por carteira por semana e só os primeiros ${DROP_RULES.eligibleFlightsPerDay} voos do dia concorrem. A moeda precisa ser pega durante o voo e o voo concluído com pelo menos ${Math.round(DROP_RULES.minQuality * 100)}% da pontuação da rota; o servidor confere o voo concluído antes de registrar o prêmio.`,
        es: `No hace falta comprar nada para participar: los vuelos son gratis (Stardust se gana jugando). Límites: ${DROP_RULES.dropsPerWalletPerWeek} moneda por billetera por semana y solo cuentan los primeros ${DROP_RULES.eligibleFlightsPerDay} vuelos del día. La moneda hay que atraparla durante el vuelo y completar el vuelo con al menos el ${Math.round(DROP_RULES.minQuality * 100)}% de la puntuación de la ruta; el servidor comprueba el vuelo completado antes de registrar el premio.`,
      },
      {
