...
| Função `chests` (baús pagos em DOG) | `functions/chests/` + `functions/_shared/` | publicada (25/09/2026); **a versão 2.5.0 (exige sessão) falta publicar** |
| Migração `20260926000000_wallet_sessions.sql` (sessões, `submit_score_v3`, progresso na nuvem) | `migrations/` | **falta aplicar** |
| Função `auth` (login com assinatura da carteira) | `functions/auth/` + `functions/_shared/auth-rules.ts` | **falta publicar** |
