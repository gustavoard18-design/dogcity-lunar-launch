...
Também cobre tutorial, molduras de nome (inclusive o visual no cartão de compartilhamento) pódio do evento, idiomas e nome do astronauta e identidade DogData, desafio, sequência, temporadas, distritos e as regras dos baús no servidor (65 testes). O CI (`.github/workflows/ci.yml`) roda typecheck, testes e build em Node 22 e 24, e publica no GitHub Pages a cada push em `main`/`master` (o build usa `base: './'`).
